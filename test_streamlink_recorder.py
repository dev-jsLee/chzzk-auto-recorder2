"""
Streamlink를 사용한 치지직 HLS 스트림 녹화 테스트
"""
...
async def test_streamlink_recording():
    """Streamlink로 치지직 스트림 녹화 테스트"""
    global recording_process, recording_active
    
    print("🧪 Streamlink 치지직 녹화 테스트")
    print("=" * 50)
    ...
    try:
        ...
        filename = f"{timestamp}_streamlink_{stream_info.title.replace(' ', '_')}.mp4"
        ...
        streamlink_cmd = [
            ...
            "--http-header", f"Cookie=NID_AUT={nid_aut}; NID_SES={nid_ses}",
        ...
        print(f"🎬 Streamlink 명령어: streamlink [options] [URL] best")
        ...
        if recording_process:
            ...
            if recording_process.returncode == 0:
                print("✅ Streamlink 실행 성공!")
            else:
                print("❌ Streamlink 실행 실패")
                print(f"stderr: {stderr[:500]}...")
                if "SSL" in stderr:
                    print("🔐 SSL/HTTPS 관련 문제일 수 있습니다")
...
if __name__ == "__main__":
    asyncio.run(test_streamlink_recording()) 